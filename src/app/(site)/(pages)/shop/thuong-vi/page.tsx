...
import { domainDatas, getDomainName } from "@/utils/provinces";
import SingleListItem from "@/components/Shop/SingleListItem";
import SingleBannerItem from "@/components/Shop/SingleBannerItem";
...
const ShopWithoutSidebarPage = () => {
  ...
  const loadProducts = async () => {
    const res = await API.get("/admin/products?category=HIGHEND", false, true);
    if (res.success) {
              ...
              <div className="flex flex-col gap-7.5">
                {selectedProducts.length === 0 ? (
                  ...
                  selectedProducts.map((item, key) => (
                    <SingleBannerItem item={item} key={key} alignLeft={key % 2 === 0} />
                  ))
                )}
              </div>
