...
function AddressesPage() {
  ...
  const handleCreate = async (data) => {
    if (!data.name || !data.phone || !data.province || !data.address)
      return;

    const res = await API.post('/address', data, true, false);
    if (res.success) {
      setIsOpen(false);
      fetchAddresses();
    }
  ...
  const handleUpdate = async (data) => {
    if (!data.id || !data.name || !data.phone || !data.province || !data.address)
      return;

    const res = await API.put('/address', data, true, false);
    if (res.success) {
      setIsOpen(false);
      fetchAddresses();
    }
  ...
  return (
    <>
        ...
        <div className="w-full bg-white shadow-lg rounded-xl">
          <div className="flex items-center justify-between py-5 px-4 sm:pl-7.5 sm:pr-6 border-b border-gray-3">
            ...
            <button
              ...
              className="text-white font-semibold p-2 rounded-xl ease-out duration-200 bg-primary hover:bg-primary-dark"
            >
              + Thêm địa chỉ mới
            </button>
          </div>

          <div className="flex flex-col p-4 gap-4 sm:p-7.5">
      ...
      <AddressModal isOpen={isOpen} closeModal={closeAddressModal} data={selected} onCreate={handleCreate} onUpdate={handleUpdate} />
    </>
  )
}
