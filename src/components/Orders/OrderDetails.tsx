...
const OrderDetails = ({ orderItem }: any) => {
  ...
  const fetchOrder = async (id) => {
    const res = await API.get(`/order/${id}`, false, false);
  ...
  return (
    <div className="flex flex-col w-full py-10 px-5 gap-4">
      <p className="font-bold text-xl text-primary w-full text-center">
        Chi tiết đơn hàng
      </p>
        ...
        <div className="items-center justify-between border-t border-gray-3 py-5 px-7.5 hidden md:flex">
          <div className="min-w-[111px]">
            <p className="text-custom-sm text-red">
              #{orderDetails.id.toString().padStart(6, "0")}
            </p>
          </div>
          <div className="min-w-[175px]">
            <p className="text-custom-sm text-dark">
              {formatDateUTC(orderDetails.createdAt)}
            </p>
          </div>

          <div className="min-w-[140px]">
            <p
              className={`inline-block text-custom-sm  py-0.5 px-2.5 rounded-[30px] capitalize ${getOrderStatusStyle(orderDetails.status)}`}
            >
              {getOrderStatusTitle(orderDetails.status)}
            </p>
          </div>

          <div className="min-w-[114px]">
            <p className="text-custom-sm text-dark">
              {orderDetails.total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} VNĐ
            </p>
          </div>
        </div>
      </div>
      ...
      {orderDetails.address && (
        <div className="px-7.5 w-full flex flex-col gap-2">
          ...
          <p className="flex items-center gap-2.5 text-custom-sm">
            ...
            Người nhận: {orderDetails.address.name}
          </p>
          ...
          <p className="flex items-center gap-2.5 text-custom-sm">
            ...
            SĐT: {orderDetails.address.phone}
          </p>
          ...
          <p className="flex gap-2.5 text-custom-sm">
            ...
            Địa chỉ: {orderDetails.address.address}, {getProvinceName(orderDetails.address.province)}
          </p>
        </div>
      )}
      ...
      <div className="px-7.5 w-full">
        ...
        <div className="flex flex-col w-full">
          ...
          {suggestItems.map((item, key) => (
            <SingleProductItem key={key} item={item} />
          ))}
          <ProductGroup cartItems={optionalItems} />
        </div>
      </div>
